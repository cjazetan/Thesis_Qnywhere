...
import 'package:qnywhere/model/businessNotifications.dart';
...
class _OwnerNotificationPageState extends State<OwnerNotificationPage> {
  ...
  void getBusinessId() async {
    final businessAccount = await FirebaseFirestore.instance
        ...
        .where('ownerId', isEqualTo: users.uid)
        .get();
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      body: StreamBuilder<List<BusinessNotifications>>(
  ...
  Widget buildNotifListView(BusinessNotifications notifications) => Card(
        elevation: 5,
  ...
  Stream<List<BusinessNotifications>> readNotifications() {
    return FirebaseFirestore.instance
        ...
        .map((snapshot) => snapshot.docs
            .where((QueryDocumentSnapshot<Object?> element) =>
                element['businessName'].toString().contains(businessId))
            .map((doc) => BusinessNotifications.fromJson(doc.data()))
            .toList());
  }
}
